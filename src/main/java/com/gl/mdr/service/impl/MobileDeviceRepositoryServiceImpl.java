...
//	UploadedFileToSyncRepository fileSyncRepository;
    ...
    public MDRGenricResponse addDevice(MobileDeviceRepository mdr) {
        ...
        try {
            ...
            logger.info("addDevice() Going to get new records for rule table default values false : " + isApproved.toString() + " and isTypeApprovedValues : " + isTypeApprovedValues);
            if (isApproved != null) {
                ...
                logger.info("addDevice() Going to get new records for rule table default values false : " + isApproved.getState() + " and isTypeApprovedValues : " + isTypeApprovedValues);
            }
        } catch (Exception e) {
            // TODO: handle exception
            logger.info("Exception addDevice() Going to get new records for rule table default values false : " + isApproved.getState() + " and isTypeApprovedValues : " + isTypeApprovedValues);
            e.printStackTrace();
        }
        try {
            ...
            if (Objects.nonNull(mdr.getManufacturerCountry()))
                newMDR.setManufacturerCountry(mdr.getManufacturerCountry());

            if (isTypeApprovedValues == 1) {
                newMDR.setIsTypeApproved(1);
                newMDR.setTrcApprovedStatus("");
                newMDR.setTrcTypeApprovedBy("");
                newMDR.setTrcApprovalDate(null);

            } else {
                newMDR.setIsTypeApproved(0);
                newMDR.setTrcApprovedStatus("Approved");
                newMDR.setTrcTypeApprovedBy("System");
                newMDR.setTrcApprovalDate(LocalDateTime.now());
            }
...
//					newMDR.setAllocationDate(mdr.getAllocationDate());
                ...
                if (Objects.nonNull(mdr.getAttachedFiles())) {
                    ...
                    String fileUploadPath = systemConfigurationDbRepository.getByTag("system_upload_filepath").getValue();
                    fileUploadPath = fileUploadPath + newMDR.getDeviceId() + "/MDR/";
    ...
    public GenericSpecificationBuilder<T> createFilter(MobileDeviceRepositoryFilterRequest mdrRequest,
        ...
        try {
            ...
            if (!isDefaultFilter) {
                ...
                if (Objects.nonNull(mdrRequest.getStartDate()) && !mdrRequest.getStartDate().equals("")) {
                    if (Objects.nonNull(mdrRequest.getMdrStatus()) &&
                            (mdrRequest.getMdrStatus() == 1 || mdrRequest.getMdrStatus() == 2 || mdrRequest.getMdrStatus() == 3)) {
                        gsb.with(new SearchCriteria("modifiedOn", mdrRequest.getStartDate(), SearchOperation.GREATER_THAN, Datatype.DATE));
                    } else {
                ...
                if (Objects.nonNull(mdrRequest.getEndDate()) && !mdrRequest.getEndDate().equals("")) {
                    if (Objects.nonNull(mdrRequest.getMdrStatus()) &&
                            (mdrRequest.getMdrStatus() == 1 || mdrRequest.getMdrStatus() == 2 || mdrRequest.getMdrStatus() == 3)) {
                        gsb.with(new SearchCriteria("modifiedOn", mdrRequest.getEndDate(), SearchOperation.LESS_THAN, Datatype.DATE));
                    } else {
                ...
                if (Objects.nonNull(mdrRequest.getUserDisplayName()) && !mdrRequest.getManufacturerCountry().equals("")) {
                    logger.info("filter getManufacturerCountry:[" + mdrRequest.getManufacturerCountry() + "]");
                    gsb.with(new SearchCriteria("manufacturerCountry", mdrRequest.getManufacturerCountry(), SearchOperation.LIKE,
                            Datatype.STRING));
                }
...
//			}
            ...
            auditTrail.setTxnId("NA");
            auditTrailRepository.save(auditTrail);
            logger.info("in MDR history audit param saved");
            return results;
        } catch (Exception e) {
            logger.error(e.getMessage(), e);
            throw new ResourceServicesException(this.getClass().getName(), e.getMessage());
    ...
    public List<String> getManufacturerCountry() {
        // TODO Auto-generated method stub
        List<String> country = mdrRepository.findManufacturerCountry();
